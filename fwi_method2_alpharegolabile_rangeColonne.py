...
def _select_known_columns(
    ...
    mode: str,
    seed: int = 42,
    col_range_start: int = 0,
    col_range_end: int = None,
    col_range_start_2: int = None,   # ← nuovo
    col_range_end_2: int = None,     # ← nuovo
) -> np.ndarray:

    if col_range_end is None:
        col_range_end = n_cols_total - 1

    # Se il secondo range è definito, costruisci il pool come unione dei due
    if col_range_start_2 is not None and col_range_end_2 is not None:
        pool_1 = np.arange(col_range_start, col_range_end + 1)
        pool_2 = np.arange(col_range_start_2, col_range_end_2 + 1)
        pool = np.concatenate([pool_1, pool_2])
        pool = np.unique(pool)  # rimuove eventuali duplicati
    else:
        pool = np.arange(col_range_start, col_range_end + 1)

    if n_known > len(pool):
        raise ValueError(
            f"n_known_cols ({n_known}) > colonne disponibili nel pool ({len(pool)})"
        )

    if mode == "spaced":
        # Prende n_known indici equidistanziati dentro il pool
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.n_known_cols is not None:
        ...
        known_col_idx = _select_known_columns(Nx, n_known, mode, seed=args.col_seed, col_range_start=args.col_range_start, col_range_end=args.col_range_end,
                                              col_range_start_2=args.col_range_start_2, col_range_end_2=args.col_range_end_2)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--col_range_start", type=int, default=0,
                    help="Indice colonna iniziale del range known (default=0, tutto il dominio)")
    parser.add_argument("--col_range_end",   type=int, default=None,
                    help="Indice colonna finale del range known (default=None, tutto il dominio)")

    parser.add_argument("--col_range_start_2", type=int, default=None,
                    help="Inizio del secondo range di colonne (opzionale)")
    parser.add_argument("--col_range_end_2",   type=int, default=None,
                    help="Fine del secondo range di colonne (opzionale)")
